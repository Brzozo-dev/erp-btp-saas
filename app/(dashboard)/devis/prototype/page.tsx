...
import { ArrowLeft, Printer, FileDown, ChevronDown, ChevronRight, Info, Plus, Trash2, X, Check, Search } from 'lucide-react';
...
function SubItemRow({ item, onCoeffChange, onTvaChange, onPriceChange, onQtyChange, onDelete }: { item: SubItem; onCoeffChange: (id: string, coeff: number) => void; onTvaChange: (id: string, taux: number) => void; onPriceChange: (id: string, newPrice: number) => void; onQtyChange: (id: string, qty: number) => void; onDelete: (id: string) => void }) {
    ...
    return (
        <tr className={`${config.stripeBg} border-l-3 ${config.borderColor} transition-colors hover:brightness-95 group`} style={{ borderLeftWidth: '3px' }}>
...
function OuvrageRow({ ouvrage, onCoeffChange, onTvaChange, onPriceChange, onQtyChange, onDeleteLine, onAddArticle, onDeleteOuvrage, onUpdateOuvrage }: {
    ...
    return (
        <>
            <tr className="bg-white hover:bg-gray-50/80 border-b border-gray-200 transition-colors group">
                <td className="py-3 pl-4 pr-2">
                    <div className="flex items-start gap-2">
                        <button onClick={() => setExpanded(!expanded)} className="mt-0.5 text-gray-400 hover:text-gray-700 transition-colors flex-shrink-0 cursor-pointer">
                ...
                <td className="py-3 px-3 text-right font-semibold text-sm text-gray-900 tabular-nums align-top">{formatMontant(ouvrage.quantite)}</td>
                <td className="py-3 px-2 text-center font-medium text-sm text-gray-600 align-top">{ouvrage.unite}</td>
            ...
            {expanded && ouvrage.sousLignes.map((sl) => (
                        ...
                        <div className="bg-white rounded-xl border shadow-sm p-6">
                            <div className="grid grid-cols-2 gap-8">
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-gray-900">16 RUE ARAGO</p>
                                    <p className="text-sm text-gray-600">44240 LA CHAPELLE SUR ERDRE</p>
                                </div>
                                <div>
                                    <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Client</h3>
                                    <p className="text-sm font-bold text-gray-900">GCC PAYS DE LA LOIRE</p>
                                    <p className="text-sm text-gray-600">16 RUE ARAGO</p>
                                    <p className="text-sm text-gray-600">44240 LA CHAPELLE SUR ERDRE</p>
                                </div>
                            </div>
                        </div>
